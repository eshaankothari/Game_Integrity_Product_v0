...
df["z3"] = -z(df["ou_ratio"])                      # ratio < 1 -> high
...
# --- equal-weighted average of the components present on each row ---
df["suspicious_score"] = df[["z1", "z2", "z3"]].mean(axis=1)     # nan-skips z2 when line moved
df["zscore_basis"] = f"test_set (n={len(df)})"

df = df.sort_values("suspicious_score", ascending=False)
cols = ["player", "group", "time", "line_move_pct", "under_move_pct", "ou_ratio",
        "z1", "z2", "z3", "suspicious_score", "zscore_basis"]
